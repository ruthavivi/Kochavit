...
type
  ...
  public
    Problem, Oved, NoBkr, Km, PolisaN, Rem: String;
...
implementation
uses
   MainDM, GnrlBigDM, ImportPhoto, ScanImport, ImportFile, ShellAPI;
{$R *.dfm}
...
begin
  ...
  if (edPolisaN.Text = '') then PolisaN := 'NULL' else PolisaN := edPolisaN.Text;
  if (edRem.Text = '') then
    Rem := 'NULL'
  else
  begin
...
procedure TfrmToDoDlg.acShowDocExecute(Sender: TObject);
var
  FolderName: String;
begin
  inherited;
  FolderName := ExtractFilePath(Application.ExeName)+'Documents\Car\'+
    dmMain.qrWrnCarCarId.AsString;
