...
begin
  ...
  StatusBar1.Panels[0].Text := ' סה"כ חשבונות בדו"ח:' + dmBillFilter.TotalSelectRec +
      ' מתוך ' + dmBillFilter.TotalRec + ' שהם: ' + Procent + '%';
end;
...
begin
  inherited;
  if SelectFields('Bl', dmBillFilter.qrFilter) then
    if GetExprtFile(FileName, 'Excel', 2) then
      SaveToExcel(dmBillFilter.qrFilter, FileName);
end;
