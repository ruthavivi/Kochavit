...
//ALTER TABLE `clients` ADD `hp` VARCHAR(60) NULL AFTER `emailX`
var
  I, RecNum: Integer;
begin
  try
...
//ALTER TABLE `cars` ADD `cod_secure` VARCHAR(60) NULL AFTER `car_shildaN`, ADD `remark` TEXT NULL AFTER `cod_secure`
var
  I, RecNum: Integer;
begin
  try
    ...
    fdQuery.SQL.Text := 'INSERT INTO cars (car_id, client_id, inspector_id, car_number, ' +
            ...
            'phone, car_brand, car_model, car_year, car_spido, car_atar, car_shildaN, cod_secure, remark) '+
            'VALUES (:car_id, :client_id, :inspector_id, :car_number, ' +
            ':car_type, :car_groupid, :car_cod, :car_weightT, :car_weightS, :car_weightR, '+
            ':car_contact, :phone, :car_brand, :car_model, :car_year, :car_spido, :car_atar, :car_shildaN, :cod_secure, :remark)';

    qrGnrl.SQL.Text := 'SELECT Id, ClientId, OvedId, Number, Type, GroupId, CodCar, WeightT, WeightS, WeightR, ' +
                'Contact, Tel, Producer, Dgam, Model, Spido, Atar, ShildaN, CodSecure, Remark FROM KCar Cr ' +
    ...
    try
      ...
      begin
        ...
        fdQuery.Params[18].AsStrings[I] := qrGnrl.FieldByName('CodSecure').AsString;
        fdQuery.Params[19].AsStrings[I] := qrGnrl.FieldByName('Remark').AsString;;
