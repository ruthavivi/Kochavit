...
interface
  uses
    SysUtils, Forms, Controls, Dialogs, edbcomps;
...
begin
  ...
  Qr := TEDBQuery.Create(Application);
  with Qr do
  begin
    Screen.Cursor := crHourGlass;
    Qr.SessionName := 'Main';
    Qr.DatabaseName := 'dbnMain';
    Qr.SQL.Text := 'SELECT Cod, Shem, Ktovet, Isuv, Mikud, Contact, TelC ' +
                  'FROM KClient';
    Qr.Open;
    ...
    try
      while not Eof do
      begin
        OneRecord := FieldByName('Cod').AsString + ';' +
