...
uses
  ...
  GnrlBigDM, System.Actions, StrUtils;
...
begin
  if MessageDlgH('למחוק מסמך מסומן', mtConfirmation,
...
begin
  ...
  if not DirectoryExists(Path) then
    CreateDir(Path);
  FileName := StringReplace(dmGnrlBig.qrBigFolder.FieldByName('Name').AsString, '/', '-', [rfReplaceAll]);
  FileName := StringReplace(FileName, '/', '-', [rfReplaceAll]);
  if FileExists(Path+'\'+ FileName +'.'+ dmGnrlBig.qrBigFolder.FieldByName('Ext').AsString) then
    MessageDlgH('קובץ קיים. החלף שם.', mtWarning, [mbOK], 0, 0)
  else
  begin
    TBlobField(dmGnrlBig.qrBigFolder.FieldByName('Tofes')).SaveToFile(Path+'\'+
      FileName +'.'+ dmGnrlBig.qrBigFolder.FieldByName('Ext').AsString);
    dmGnrlBig.DeleteDoc(Table, dmGnrlBig.qrBigFolder.FieldByName('Id').AsString);
  end;
end;
...
begin
  ...
  if (Table = 'KClTofes') then
    frmSlctOther.qrSlctOther.SQL.Text := 'SELECT Id, Shem AS Main FROM KClient ' +
              'ORDER BY Shem'
  else if (Table = 'KCrTofes') then
    frmSlctOther.qrSlctOther.SQL.Text := 'SELECT Id, Number AS Main FROM KCar ' +
              'ORDER BY Number'
  else if (Table = 'KDrTofes') then
    frmSlctOther.qrSlctOther.SQL.Text := 'SELECT Id, ShemD AS Main FROM KDriver ' +
              'ORDER BY ShemD';
  try
    frmSlctOther.qrSlctOther.Open;
    if (frmSlctOther.ShowModal = mrOk) then
    begin
      if (Table = 'KClTofes') then
        dmGnrlBig.qrGnrl.SQL.Text := 'UPDATE KClTofes SET ClientId = ' +
                IntToStr(frmSlctOther.Id) + ' WHERE Id = ' +
                dmGnrlBig.qrBigFolder.FieldByName('Id').AsString
      else if (Table = 'KCrTofes') then
        dmGnrlBig.qrGnrl.SQL.Text := 'UPDATE KCrTofes SET CarId = ' +
                IntToStr(frmSlctOther.Id) + ' WHERE Id = ' +
                dmGnrlBig.qrBigFolder.FieldByName('Id').AsString
      else if (Table = 'KDrTofes') then
        dmGnrlBig.qrGnrl.SQL.Text := 'UPDATE KDrTofes SET DriverId = ' +
                IntToStr(frmSlctOther.Id) + ' WHERE Id = ' +
                dmGnrlBig.qrBigFolder.FieldByName('Id').AsString;

      dmGnrlBig.qrGnrl.ExecSQL;
    end;
    dmGnrlBig.qrBigFolder.Refresh;
  finally
    frmSlctOther.qrSlctOther.Close;
    frmSlctOther.Free;
  end;
end;
