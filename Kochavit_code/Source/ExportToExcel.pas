...
begin
  ...
  begin
    if (pFrom = 'Cars') then
      BodyTxt := 'לכבוד' +#13#10+ pClientName +#13#10#10#13+
          'מצורפת מצבת הרכבים של חברתך.' +#13#10#13#10+GetSignature
    else
      BodyTxt := 'לכבוד' +#13#10+ pClientName +#13#10#10#13+
          'מצורפת מצבת נהגים של חברתך.' +#13#10#13#10+GetSignature;
    SendByEMail(pClientName, pToEmails);
  end
  else
    ShellExecute(Handle, 'open', PChar(SavedFile) ,nil, nil, SW_SHOWNORMAL);

  Screen.Cursor := crDefault;
end;

function TfrmExportToExcel.GetSignature: string;
begin
  dmMain.qrGnrl.SQL.Text := 'SELECT Signature FROM KDefault';
  try
    dmMain.qrGnrl.Open;
...
begin
   ...
   try
    scExcelExport.ExportDataset;
    scExcelExport.SaveAs(pToFile, ffXLS);
  finally
    scExcelExport.Disconnect(True);
  end;
  Screen.Cursor := crDefault;
