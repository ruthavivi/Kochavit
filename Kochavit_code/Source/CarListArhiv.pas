...
type
  TfrmCarListArhiv = class(TfrmGnrlListArhiv)
    ...
    procedure acExcelExecuteExecute(Sender: TObject);
  private
...
begin
  StatusBar.Panels[0].Text := 'סה"כ רכבים: ' + dmCarListArhiv.CarRecNo;
end;
...
begin
  ...
  frmReturnDlg.lbHead.Caption := 'להחזיר רכב מספר ' +
        dmCarListArhiv.qrList.FieldByName('Number').AsString + ' לחברה?';
  try
...
begin
  inherited;
  if (MessageDlgH('למחוק סופית רכב מספר ' +
...
procedure TfrmCarListArhiv.acExcelExecuteExecute(Sender: TObject);
var
  FileName: String;
begin
  inherited;
  if SelectFields('Ar', dmCarListArhiv.qrList) then
    if GetExprtFile(FileName, 'Excel', 2) then
      SaveToExcel(dmCarListArhiv.qrList, FileName);
end;
