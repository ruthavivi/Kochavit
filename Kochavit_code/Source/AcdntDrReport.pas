...
begin
  ...
  StatusBar1.Panels[0].Text := ' סה"כ תאונות בדו"ח: ' + dmAcdntDrFilter.TotalSelectRec +
      ' מתוך ' + dmAcdntDrFilter.TotalRec + ' שהם: ' + Procent + '%';
end;
...
begin
  inherited;
  if SelectFields('Acd', dmAcdntDrFilter.qrFilter) then
    if GetExprtFile(FileName, 'Excel', 2) then
      SaveToExcel(dmAcdntDrFilter.qrFilter, FileName);
end;
