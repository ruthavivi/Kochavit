...
var
  Procent: String;
begin
  if (StrToInt(dmTahoFilter.TotalRec) = 0) then
    Procent := '0'
  else
    Procent := FormatFloat('#.##', StrToInt(dmTahoFilter.TotalSelectRec) /
          StrToInt(dmTahoFilter.TotalRec) * 100);
  StatusBar1.Panels[0].Text := ' סה"כ בדיקות בדו"ח: ' + dmTahoFilter.TotalSelectRec +
      ' מתוך ' + dmTahoFilter.TotalRec + ' שהם: ' + Procent + '%';
end;
...
begin
  inherited;
  frmTahoRprtSlctDlg := TfrmTahoRprtSlctDlg.Create(Self);
  frmTahoRprtSlctDlg.Show;
end;
...
begin
  inherited;
  if SelectFields('Th', dmTahoFilter.qrFilter) then
    if GetExprtFile(FileName, 'Excel', 2) then
      SaveToExcel(dmTahoFilter.qrFilter, FileName);
end;
