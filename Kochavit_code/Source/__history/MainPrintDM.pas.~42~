...
uses
  SysUtils, Classes, Forms, Controls, edbcomps, DB, frxClass, frxDBSet;
...
procedure TdmMainPrint.DataModuleDestroy(Sender: TObject);
begin
  if (ReportName = 'rpWrnCarWhLikuy') then
    qrToPrintDtl1.Close
  else if (ReportName = 'rpWrnCarWhLikuyWhTipul') then
  begin
    qrToPrintDtl1.Close;
    qrToPrintDtl2.Close;
  end;
